...
const AddSize = ({ saveCompleteEvent }) => {
    ...
    return (
        ...
        <Dialog open={state.addSize} onClose={handleClose}>
            <Typography variant={'h3'} margin={2}>Add New Size</Typography>
            <Formik
                initialValues={{
                    name: ''
                }}
                validationSchema={Yup.object().shape({
                    name: Yup.string().max(255).required('Please enter a size')
                })}
            ...
            >
                {({ errors, handleBlur, handleChange, handleSubmit, isSubmitting, touched, values }) => (
                    <form noValidate onSubmit={handleSubmit}>
                        <DialogContent>
                            <TextField name={'name'} label='Size' variant='outlined' value={values.name}
                                       onBlur={handleBlur}

...
const AddColor = ({ saveCompleteEvent }) => {
    ...
    return (
        ...
        <Dialog open={state.addColor} onClose={handleClose}>
            <Typography variant={'h3'} margin={2}>Add New Color</Typography>
            <Formik
                initialValues={{
                    name: '',
                    code: ''
                ...
                validationSchema={Yup.object().shape({
                    name: Yup.string().max(255).required('Please enter color name.'),
                    code: Yup.string().max(255).required('Please select color code.')
                })}
            ...
            >
                {({ errors, handleBlur, handleChange, handleSubmit, isSubmitting, touched, values }) => (
                    <form noValidate onSubmit={handleSubmit}>
                        <DialogContent>
                            <TextField name={'name'} label='Color name' variant='outlined' value={values.name}
                                       onBlur={handleBlur}
                                       onChange={handleChange} />
                            <TextField name={'code'} type={'color'} label='Code' sx={{ width: 50 }} value={values.code}
                                       onBlur={handleBlur}
                                       onChange={handleChange} />

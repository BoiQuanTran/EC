    ...
    if (a) {
        return (
            ...
            <Dialog open={state.updateCategory} onClose={handleClose}>
                <Typography variant={'h3'} margin={2}>Update Category</Typography>
                <Formik
                    initialValues={{
                        name: a.name
                    }}
                    validationSchema={Yup.object().shape({
                        name: Yup.string().max(255).required('Please enter catrgory name.')
                    })}
                ...
                >
                    {({ errors, handleBlur, handleChange, handleSubmit, isSubmitting, touched, values }) => (
                        <form noValidate onSubmit={handleSubmit}>
                            <DialogContent>
                                <TextField name={'name'} label='Catrgory name' variant='outlined' value={values.name}
                                           onBlur={handleBlur}
